#!/usr/bin/env bash
# Cron job 4 — sincronização horária
# Schedule: 0 * * * *  (job: email-cron-4-hourly-sync)
...
echo "[] Executando email-cron-4-hourly-sync..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 4}' && echo " OK" || echo " ERRO"
