#!/usr/bin/env bash
# Cron job 6 — fechamento mensal
# Schedule: 0 0 1 * *  (job: email-cron-6-monthly-close)
...
echo "[] Executando email-cron-6-monthly-close..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 6}' && echo " OK" || echo " ERRO"
