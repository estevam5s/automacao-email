#!/usr/bin/env bash
# Cron job 2 — limpeza diária 02h
# Schedule: 0 2 * * *  (job: email-cron-2-daily-cleanup)
...
echo "[] Executando email-cron-2-daily-cleanup..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 2}' && echo " OK" || echo " ERRO"
