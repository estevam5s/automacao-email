#!/usr/bin/env bash
# Cron job 1 — a cada 6 horas
# Schedule: 0 */6 * * *  (job: email-cron-1-6h-heartbeat)
...
echo "[] Executando email-cron-1-6h-heartbeat..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 1}' && echo " OK" || echo " ERRO"
