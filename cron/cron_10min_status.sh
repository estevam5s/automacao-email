#!/usr/bin/env bash
# Cron job 3 — status a cada 10min
# Schedule: */10 * * * *  (job: email-cron-3-10min-status)
...
echo "[] Executando email-cron-3-10min-status..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 3}' && echo " OK" || echo " ERRO"
