#!/usr/bin/env bash
# Cron job 9 — reindexação semanal
# Schedule: 0 4 * * 0  (job: email-cron-9-weekly-reindex)
...
echo "[] Executando email-cron-9-weekly-reindex..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 9}' && echo " OK" || echo " ERRO"
