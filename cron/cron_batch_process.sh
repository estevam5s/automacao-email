#!/usr/bin/env bash
# Cron job 10 — batch 09h 15h 21h
# Schedule: 0 9,15,21 * * *  (job: email-cron-10-batch-process)
...
echo "[] Executando email-cron-10-batch-process..."

curl -s -X POST "/rest/v1/table_cron"   -H "apikey: "   -H "Authorization: Bearer "   -H "Content-Type: application/json"   -H "Prefer: return=minimal"   -d '{"num": 10}' && echo " OK" || echo " ERRO"
